...
async function main() {
  ...
  // ---------- 备份 ----------
  await cdp.evaluate(`return await window.shopilot.store.create({ name:'KEEP-M2', platform:'快手小店' });`)
  ...
  await cdp.evaluate(`return await window.shopilot.store.create({ name:'GONE-M2', platform:'微信小店' });`)
