...
async function main() {
  ...
  check('「达人广场」是独立页签：只列已支持平台、占位符=内置实测地址、无横向溢出',
    ...
    settingsTabs.square.names.join(',') === '抖店,微信小店,快手小店' &&
  ...
  const squareOverride = await cdp.evaluate(`
    ...
    // 批量邀约步骤现在按“每批”放在 loop 内；验收关注的是实际生成的
    // navigate/wait/gate 顺序，而不是把实现细节误当成顶层步骤。
    const allSteps = [];
    const flatten = list => (list || []).forEach(s => {
      allSteps.push(s);
      if (Array.isArray(s?.input?.steps)) flatten(s.input.steps);
    });
    flatten(steps);
    const navStep = allSteps.find(s => s.type === 'navigate');
    const pageStep = allSteps.find(s => s.type === 'waitForPage');
    const gateStep = allSteps.find(s => s.type === 'waitForUserConfirmation');
      ...
      out.navUrl = navStep?.input?.url || null;
      out.urlIncludes = pageStep?.input?.urlIncludes || null;
      out.hasAiStep = allSteps.some(s => s.type === 'aiGenerate');
