 ...
 * 用法：node tools/release/update-online-drill.js [旧版win-unpacked目录] [新版本号] [旧版本号]
 *       默认：release-drill-backup/win-unpacked  0.1.1-beta.1  0.1.0
 * CDP：旧版 9247 / 安装版 9248；日志：logs/update-online-drill.log
 */
const fs = require('fs')
const os = require('os')
...
function report(passed) {
  ...
  try { fs.mkdirSync(LOG_DIR, { recursive: true }); fs.writeFileSync(path.join(LOG_DIR, 'update-online-drill.log'), JSON.stringify(out, null, 2)) } catch { /* ignore */ }
...
async function main() {
  ...
  try {
    ...
    const installedExe = path.join(installDir, 'ShopPilot.exe')
