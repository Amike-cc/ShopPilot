 ...
 * 用法：node tools/release/update-runner.js    （CDP 9241 / feed 9245，独立临时 userData）
 */
...
function report(passed) {
  ...
  try { fs.mkdirSync(LOG_DIR, { recursive: true }); fs.writeFileSync(path.join(LOG_DIR, 'update-run1.log'), JSON.stringify(out, null, 2)) } catch { /* ignore */ }
