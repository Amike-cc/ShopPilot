 ...
 * 用法：node tools/ui/ui-shot.js [面板名(env|bookmarks|downloads|tasks|open|ctx|trash)] [--real]
 *
...
async function main() {
  ...
  try {
    ...
    if (panel === 'trash') {
      ...
      await cdp.shot(path.join(ARTIFACT_DIR, 'ui-trash-open.png'))
      ...
      await cdp.shot(path.join(ARTIFACT_DIR, 'ui-trash-closed.png'))
      console.log('\n=== 回收站链路 ===')
      ...
      if (cdp.console.length) console.log('\n=== 渲染层报错 ===\n' + cdp.console.slice(-8).join('\n'))
      console.log('\n截图: artifacts/ui/ui-trash-open.png / artifacts/ui/ui-trash-closed.png')
      cdp.close()
      return
    }
    ...
    const shot = path.join(ARTIFACT_DIR, `ui-${panel}-shot.png`)
    await cdp.shot(shot)
    console.log('\n截图: ' + shot)
    const full = path.join(ARTIFACT_DIR, `ui-${panel}-full.png`)
