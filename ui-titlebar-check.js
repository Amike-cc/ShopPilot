...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(ROOT, 'ui-titlebar-full.png'), Buffer.from(shot.data, 'base64'))
    ...
    fs.writeFileSync(path.join(ROOT, 'ui-titlebar-top.png'), Buffer.from(shot2.data, 'base64'))
    console.log('已保存 ui-titlebar-full.png ui-titlebar-top.png')

    results.passed = results.checks.every(c => c.ok)
    fs.writeFileSync(path.join(ROOT, 'ui-titlebar-check.log'), JSON.stringify(results, null, 2))
    ws.close()
    console.log(results.passed ? 'ALL PASS' : 'HAS FAILURES')
  } catch (e) {
    results.error = String(e?.message || e)
    try { fs.writeFileSync(path.join(ROOT, 'ui-titlebar-check.log'), JSON.stringify(results, null, 2)) } catch {}
