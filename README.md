...
**版本**: v0.4.34 · **构建标签**: `INTERNAL_BUILD`（未做代码签名，§21.5）
**里程碑**: M0 技术验证 ✅ · M1 工作台核心 ✅ · M2 环境/代理/备份 ✅ · M3 任务辅助 ✅ · M4 发布工程 ✅ · 自动更新（§21）✅
**验收**: 0.4.34 构建复跑通过：单测 183/183、发票中心营业执照本地端到端 26/26、真实店铺库实测 7/7（用完即还原）；0.4.33 构建通过：M3 113/113、店铺拖动排序 10/10、抖店邀约 21/21、M4 打包态核心 19/19；本机应用控制策略阻止执行 electron-builder 重写后的测试 EXE，因此 M4 安装/升级/卸载阶段未在本机复跑。M1/M2/安全/更新链路的最近结果见 `release/release.json`。M2 29+8 是 0.1.0 构建上的结果，其后未复跑（改动未触及代理/指纹那条路径）。
