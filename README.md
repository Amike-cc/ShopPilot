...
**版本**: v0.4.25 · **构建标签**: `INTERNAL_BUILD`（未做代码签名，§21.5）  
...
| [RELEASE_NOTES.md](./RELEASE_NOTES.md) | 0.4.25 变更说明与回滚方式 |
...
- **达人邀约（抖店 + 微信小店，平台流程相互独立）**：
  ...
  - **独立于任务列表**：任务面板不再有「任务列表」页签——任务引擎只作为两个功能的执行机制（达人邀约 / 数据中心采集）；邀约运行记录在邀约面板自带的「邀约记录」里（状态/步骤结果/门禁日志/发送截图留档，近 5 次），采集结果在数据中心按店铺展示。
...
- **发票中心（各平台发票页采集）**：抓取各店铺**待开票信息**并汇总展示，支持跨店铺合计、搜索、金额排序与 CSV 导出（UTF-8 BOM + 正确转义）。按**开票方向**分别采集与分组展示（同一发票页上「给平台开票 / 给买家开票 / 申请平台开票」是不同数据，只抓默认页签必漏）；方向可能在**另一个站点**（拼多多给平台开票在资金中心），也可能是**卡片而非表格**（标签 + 值）。采集是全只读步骤、不做任何开票动作。锚点全部实测登记在 `packages/shared/src/constants/invoice.ts`，未实测的平台如实说明"抓不了"，绝不猜选择器。
...
```powershell
pnpm exec vitest run        # 单测 73 项（会话包 / 邀约档案与步骤 / 发票档案、多方向采集步骤、表格行清洗、CSV 转义）
