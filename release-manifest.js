...
const manifest = {
  ...
  dbSchemaVersion: 2,
  acceptanceSuites: [
    'node m1-runner.js（90 项，含左栏收起/展开、回收站徽标计数、右栏收起/展开、国内四平台目录与平台入口链路）',
  ...
  // 只声明本版（0.1.1）构建上实际复跑的套件；未复跑的不计入，避免把历史结果冒充本版结论
  acceptanceSummary: '本版构建复跑通过：M1 90 + 安全 44 + 更新链路 16 + 单测 9 + M4 13 阶段（含 19/3/4 分项）。M2（代理/指纹）与 M3（任务引擎）未在本版构建上复跑——本次改动未触及这两条代码路径。',
  artifacts: files,
  ...
  knownBoundaries: [
    '未做代码签名，构建标记 INTERNAL_BUILD；自动更新经 GitHub Releases（Amike-cc/ShopPilot，v0.1.1 已发布，仓库 public）分发，更新包仅做 SHA-512 哈希校验、无签名校验；打包态匿名在线检查已实测通过',
