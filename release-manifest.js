...
const manifest = {
  ...
  acceptanceSuites: [
    ...
    'node m3-runner.js（61 项，含任务面板二级页签与达人邀约占位、确认门禁跨面板可见性、右栏收起守卫与对话框布局守卫）',
  ...
  // 只声明本版（0.1.3）构建上实际复跑的套件；未复跑的不计入，避免把历史结果冒充本版结论
  acceptanceSummary: '本版构建复跑通过：M1 100 + M3 61 + 安全 44 + 更新链路 16 + 单测 9 + M4 13 阶段（含 19/3/4 分项）。M2（代理/指纹）未在本版构建上复跑——本次改动未触及该路径。',
  artifacts: files,
  ...
  knownBoundaries: [
    '未做代码签名，构建标记 INTERNAL_BUILD；自动更新经 GitHub Releases（Amike-cc/ShopPilot，v0.1.3 已发布，仓库 public）分发，更新包仅做 SHA-512 哈希校验、无签名校验；打包态匿名在线检查已实测通过',
