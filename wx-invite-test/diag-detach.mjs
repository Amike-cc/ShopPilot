/** 诊断：为什么店铺视图未挂载（0×0）——看渲染层弹层状态、中间栏在显示什么、视图几何。 */
...
console.log('渲染层视口:', await ev(`innerWidth + 'x' + innerHeight`))
console.log('中间栏在显示:', await ev(`(() => {
  if (document.querySelector('.welcome')) return 'welcome（未选中店铺）'
  const vp = document.querySelector('.viewport, [class*=viewport]')
  return vp ? 'viewport 存在 ' + JSON.stringify(vp.getBoundingClientRect()) : '(既无 welcome 也无 viewport)'
})()`))
console.log('打开的弹层:', await ev(`JSON.stringify({
  settings: !!document.querySelector('[data-test=settings-modal]'),
  invoice: !!document.querySelector('[data-test=invoice-center-modal]'),
  data: !!document.querySelector('[data-test=data-center-modal]'),
  create: !!document.querySelector('[data-test=create-dialog]'),
  trash: !!document.querySelector('[data-test=trash-drawer]')
})`))
console.log('当前显示的店铺:', await ev(`JSON.stringify({
  displayedStoreId: window.__pinia?.workbench?.displayedStoreId ?? null
})`))
// 直接问主进程视图尺寸
console.log('\n===== 快手页面视口 =====')
const l2 = await (await fetch(`http://127.0.0.1:${PORT}/json/list`)).json()
const pg = l2.filter(x => x.type === 'page' && x.url.includes('kwaixiaodian') && !x.url.includes('login.')).pop()
if (pg) {
  ...
  console.log('地址:', await q2(`location.href.slice(0,90)`))
